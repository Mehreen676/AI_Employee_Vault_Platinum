...
│   ├── main.py                     # FastAPI app: /status /health /queue/* /evidence/* /metrics
│   │                               #   POST /slack/webhook  POST /whatsapp/webhook
│   │                               #   GET  /monitoring/health  POST /reports/ceo
│   ├── agent/
...
│
├── integrations/                   # Production integration modules (new in v1.5)
│   ├── gmail_integration.py        # Gmail API: OAuth 2.0 + stub fallback → Needs_Action/email/
│   ├── slack_integration.py        # Slack Events API webhook → vault task
│   ├── whatsapp_integration.py     # Twilio WhatsApp webhook → vault task (TwiML response)
│   ├── voice_interface.py          # Voice command: SpeechRecognition + stdin fallback
│   └── ai_decision_logger.py       # Per-task reasoning / tool / risk log → AI_DECISION_LOG.md
│
├── monitoring/                     # Self-monitoring system
│   └── health_monitor.py           # Component health checker → SYSTEM_HEALTH_REPORT.md
│
...
│   ├── generate_ceo_report.py      # CEO daily report → Evidence/CEO_REPORT.md
...
│   ├── PLATINUM_ARCHITECTURE_V2.md # 7-layer Mermaid architecture diagram
│   ├── PLATINUM_ARCHITECTURE_V2.png
│   ├── AI_DECISION_LOG.md          # Per-task AI reasoning / tool / risk log
│   ├── SIMULATION_REPORT.md        # 50-task company simulation report
│   ├── CEO_REPORT.md               # CEO daily business report (runtime output)
│   ├── SYSTEM_HEALTH_REPORT.md     # Live component health check report
│   ├── GMAIL_INTEGRATION_LOG.md    # Gmail webhook activity log
│   ├── SLACK_INTEGRATION_LOG.md    # Slack webhook activity log
│   ├── WHATSAPP_INTEGRATION_LOG.md # Twilio WhatsApp activity log
│   ├── VOICE_COMMAND_LOG.md        # Voice interface command log
│   ├── MCP_PROOF.md
│   ├── HISTORY_PROOF.md
│   ├── RUN_CHECKLIST.md
...
├── simulate_company_day.py         # 50-task company simulation → Evidence/SIMULATION_REPORT.md
...
|---|---|---|
| [Evidence/PLATINUM_ARCHITECTURE_V2.md](Evidence/PLATINUM_ARCHITECTURE_V2.md) | Full Mermaid 7-layer architecture diagram, claim-by-move protocol, permission matrix | Already committed — open in any Markdown renderer |
...
| [Evidence/AI_DECISION_LOG.md](Evidence/AI_DECISION_LOG.md) | Per-task AI reasoning, tool selection, risk level, outcome — SHA-256 compatible | Auto-generated by `integrations/ai_decision_logger.py` |
| [Evidence/SIMULATION_REPORT.md](Evidence/SIMULATION_REPORT.md) | 50-task company day simulation across 6 business zones | `python simulate_company_day.py --n 50` |
| [Evidence/CEO_REPORT.md](Evidence/CEO_REPORT.md) | CEO daily business report: KPIs, vault state, top action types | `python scripts/generate_ceo_report.py` or `POST /reports/ceo` |
| [Evidence/SYSTEM_HEALTH_REPORT.md](Evidence/SYSTEM_HEALTH_REPORT.md) | Live component health: cloud_agent, gmail_watcher, local_executor, MCP tools | `GET /monitoring/health` or `python -m monitoring.health_monitor` |
| [Evidence/GMAIL_INTEGRATION_LOG.md](Evidence/GMAIL_INTEGRATION_LOG.md) | Gmail OAuth/stub webhook activity — every email ingested | Auto-generated by `integrations/gmail_integration.py` |
| [Evidence/SLACK_INTEGRATION_LOG.md](Evidence/SLACK_INTEGRATION_LOG.md) | Slack Events API webhook activity log | Auto-generated by `integrations/slack_integration.py` |
| [Evidence/WHATSAPP_INTEGRATION_LOG.md](Evidence/WHATSAPP_INTEGRATION_LOG.md) | Twilio WhatsApp webhook activity — TwiML responses | Auto-generated by `integrations/whatsapp_integration.py` |
| [Evidence/VOICE_COMMAND_LOG.md](Evidence/VOICE_COMMAND_LOG.md) | Voice command processing log (mic or stdin mode) | Auto-generated by `integrations/voice_interface.py` |
...
The following endpoints were added to `backend_api/main.py` and `hf_space_backend/main.py` as part of the integration expansion:

| Method | Path | Description |
|---|---|---|
| `GET` | `/metrics` | System performance metrics: total tasks, success rate, avg latency, queue snapshot |
| `POST` | `/slack/webhook` | Slack Events API webhook — creates vault task from any Slack message |
| `POST` | `/whatsapp/webhook` | Twilio WhatsApp webhook — accepts form-encoded POST, returns TwiML XML |
| `GET` | `/monitoring/health` | Live component health: cloud_agent, gmail_watcher, local_executor + MCP tool imports |
| `POST` | `/reports/ceo` | Trigger CEO daily report generation (`scripts/generate_ceo_report.py`) |

**Dashboard metrics panel** (auto-refreshes every 30 s):
- **Processed Tasks** — total from execution_log.json
- **Failed Tasks** — error results + rejected count
- **Pending Approvals** — live Pending_Approval + Waiting_Approval queue count
- **Avg Execution Time** — inter-task latency estimate in ms

**Integrations summary:**

| Integration | Source | Evidence File |
|---|---|---|
| Gmail (OAuth 2.0) | `integrations/gmail_integration.py` | `Evidence/GMAIL_INTEGRATION_LOG.md` |
| Slack Events API | `integrations/slack_integration.py` | `Evidence/SLACK_INTEGRATION_LOG.md` |
| Twilio WhatsApp | `integrations/whatsapp_integration.py` | `Evidence/WHATSAPP_INTEGRATION_LOG.md` |
| Voice Interface | `integrations/voice_interface.py` | `Evidence/VOICE_COMMAND_LOG.md` |
| AI Decision Logger | `integrations/ai_decision_logger.py` | `Evidence/AI_DECISION_LOG.md` |
| Self-Monitoring | `monitoring/health_monitor.py` | `Evidence/SYSTEM_HEALTH_REPORT.md` |
| CEO Report | `scripts/generate_ceo_report.py` | `Evidence/CEO_REPORT.md` |
| Company Simulation | `simulate_company_day.py` | `Evidence/SIMULATION_REPORT.md` |
