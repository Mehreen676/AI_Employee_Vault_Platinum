#!/usr/bin/env python3
"""
Generate Evidence/PLATINUM_ARCHITECTURE_V2.png
AI Employee Vault – Platinum Tier | v1.4.0
Dark-themed, colorful, judge-friendly architecture diagram.
Output: 1600 × 2600 px at 100 DPI
"""
...
# ── Use emoji-capable font stack ──────────────────────────────
plt.rcParams['font.family'] = ['Segoe UI Emoji', 'Segoe UI', 'DejaVu Sans']

# ── Paths ─────────────────────────────────────────────────────
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT_PNG   = os.path.join(REPO_ROOT, 'Evidence', 'PLATINUM_ARCHITECTURE_V2.png')
...
def rbox(x, y, w, h, fc, ec, lw=2.0, alpha=0.92, pad=0.10):
    ax.add_patch(FancyBboxPatch(
        ...
        linewidth=lw, alpha=alpha, zorder=2, clip_on=False,
    ))
...
def down_arrow(cx, y_from, y_to):
    ax.annotate(
        '', xy=(cx, y_to + 0.07), xytext=(cx, y_from - 0.07),
        arrowprops=dict(arrowstyle='-|>', color=ARROW_C,
                        lw=3.8, mutation_scale=34),
        zorder=5,
    )
...
LCOMP_H = 1.46
LFOOTER = 0.14
LTOTAL  = LHEADER + LGAP + LCOMP_H + LFOOTER   # ≈ 2.54"

ARROW_H = 0.72

# Title zone takes top 1.45"
y_cur = FH - 1.45
y_bottoms = []
for _ in range(7):
    y_cur -= LTOTAL
...
def draw_layer(y_bot, key, badge, title, components):
    """
    badge     : short prefix / icon label for the header
    title     : full header text (badge is prepended only if different)
    components: list of (short_badge, two_line_label)
    """
    p = LAYERS[key]

    # Outer ghost box
    rbox(MARGIN, y_bot, LW, LTOTAL,
         fc=p['header'], ec=p['border'], lw=2.5, alpha=0.18, pad=0.12)

    # Header band
    h_y = y_bot + LFOOTER + LCOMP_H + LGAP
    h_h = LHEADER
    rbox(MARGIN, h_y, LW, h_h,
         fc=p['header'], ec=p['border'], lw=2.2, alpha=0.97, pad=0.10)
    txt(MX, h_y + h_h / 2, title, sz=14.5, c=WHITE, bold=True)
    ...
    for i, (ico, label) in enumerate(components):
        ...
        rbox(cx, c_y, c_w, LCOMP_H,
             fc=p['inner'], ec=p['border'], lw=1.8, alpha=0.90, pad=0.08)
        txt(cx + c_w / 2, c_y + LCOMP_H / 2,
...
txt(MX, TITLE_Y,
    ...
    sz=22, c=TITLE_C, bold=True)
txt(MX, TITLE_Y - 0.65,
    'Distributed AI Task Management  |  v1.4.0  |'
    '  Oracle OCI (me-dubai-1) + HuggingFace Spaces',
    sz=11.5, c=SUB_C, bold=False)
ax.axhline(TITLE_Y - 1.05,
           xmin=MARGIN / FW, xmax=(FW - MARGIN) / FW,
...
# ── Footer ────────────────────────────────────────────────────
foot_y = Y_SVCS - 0.48
ax.axhline(foot_y + 0.22,
           xmin=MARGIN / FW, xmax=(FW - MARGIN) / FW,
           color=TITLE_C, lw=1.0, alpha=0.28, zorder=1)
txt(MX, foot_y,
    'AI Employee Vault – Platinum Tier  |  Claim-by-Move Protocol  '
    '|  SHA-256 Audit Chain  |  Oracle Cloud (me-dubai-1)',
